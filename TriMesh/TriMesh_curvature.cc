...
namespace Graphics{ namespace TriMeshSpace{
...
#pragma omp parallel for
	...
	// By Sangwook
	pBasis1.clear(); pBasis1.resize(nv);
	pBasis2.clear(); pBasis2.resize(nv);
	pBasis3.clear(); pBasis3.resize(nv);
	for(int i=0; i<nv; i++)
	{
		pBasis1[i] = pdir1[i];		// local basis ˜ˌ
		pBasis2[i] = pdir2[i];
		pBasis3[i] = normals[i];	// vertex normals
	}
